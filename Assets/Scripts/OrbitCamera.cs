// Copyright (C) Yunus Kara 2019-2024. All Rights Reserved.
...
public class OrbitCamera : MonoBehaviour
{
    ...
    public bool mOrbitMode = false;
    public void setTarget(Transform t) { Target = t; }
    public void setSpring(float s) { Spring = s; }
    public void setDistance(float d) { Distance = d; }
    public bool innerCamera = false;

    private float innerCameraSpring = 25f;
    private float innerCameraDistance = 0.01f;
    private float innerCameraOffset = 0.75f;
    private float innerCameraTilt = 0f;
    private float outerCameraSpring = 2.5f;
    private float outerCameraDistance = 6f;
    private float outerCameraOffset = 1f;
    private float outerCameraTilt = 12f;
    ...
    void Update()
    {
        ...
        {
            Spring = innerCamera ? outerCameraSpring : innerCameraSpring;
            Distance = innerCamera ? outerCameraDistance : innerCameraDistance;
            TargetYOffset = innerCamera ? outerCameraOffset : innerCameraOffset;
            TiltAngle = innerCamera ? outerCameraTilt : innerCameraTilt;
        ...
        if (Target != null)
        {
            Quaternion rot = Quaternion.Euler(mCurrentTilt, mRotationY + mRotationOffset, 0);
    ...
    void FixedUpdate()
    {
        ...
        if (Target != null)
        {
            ...
            if (DriftMode)
            {
                ...
                float speed = (delta.magnitude / Time.fixedDeltaTime) * 3.6f;
